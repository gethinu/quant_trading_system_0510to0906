...
class TestMainExitCodes:
    ...
    def test_notify_called_with_flag(self, tmp_path: Path, monkeypatch):
        ...
        _write_signal(data, "20260721")
        called = {"n": 0}
        # _notify_stale(result, root, now): root/now は失敗持ち越し (logs/ への
        # 再送キュー) 用。stub もこの signature に合わせる。
        monkeypatch.setattr(
            cdf,
            "_notify_stale",
            lambda _r, _root, _now: called.__setitem__("n", 1),
        )
        cdf.main(
            [
                "--results-dir",
