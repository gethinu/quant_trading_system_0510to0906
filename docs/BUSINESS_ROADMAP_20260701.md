...
[Phase 1] 自家用 無人化          [Phase 2] β配信 (無償/招待)        [Phase 3] 課金事業化
 ...
 ntfy + email 2段            per-subscriber routing              請求 / 契約 / 投資助言業確認
 Vercel dashboard (自分用)         private dashboard / β招待           tier別 dashboard / API 提供
```

各 Phase の技術要素は **同じ interface** の下で差し替える:
- 配信: `common/publishers/` の `Publisher` ABC (`send`/`is_configured`)。
  Phase 1 は **ntfy.sh (primary) + SendGrid Email (backup)** を実装、LINE は Phase 2 skeleton。
  **Discord / LINE Messaging API / SMS / Slack は同 ABC の下でいつでも追加可能** (registry 登録のみ)。
- 配信 chain: `common/publishers/registry.py` の `PublisherRegistry` (primary→backup fallback / 並列)。
- 配信先解決: `scripts/publish_signals.py` (`--publisher ntfy|email|all` / `--fallback`)。
- シグナル schema: `today_signals_*.json` v1.0 (`run_id` で重複配信/発注を検出, `meta.publish_status` で配信監視)
...
- **06:00 JST daily** に Windows Task Scheduler → `scripts/daily_pipeline.ps1` が
  cache → signals → coverage → **narrator** → publish を無人で通す。
- **AI narrator (事業差別化の核)**: `common/narrator.py` が当日シグナルを Claude API で
  headline + 2 段落 summary + 各 symbol の 1 行 reason に翻訳し `meta.narrative` へ付与。
  「signals だけ」の他社に対し解説付きで配信/表示することで subscriber tier の礎を作る。
  ANTHROPIC_API_KEY 未設定でも fail-safe (空 narrative + WARN) で pipeline は止まらない。
...
- ntfy.sh push (iPhone) に system 別 summary + gate 生存率 WARN が届き、失敗時は email backup。
...
| 配信抽象 | `common/publishers/` (`Publisher` ABC + ntfy/email 実装 + registry, LINE/Twitter/Substack skeleton) |
| AI narrator | `common/narrator.py` (`SignalNarrator`, Claude API, fail-safe, hallucination cross-check) + `scripts/narrate_signals.py` |
| 配信 CLI | `scripts/publish_signals.py` (`--publisher ntfy\|email\|all` / `--fallback`)。ntfy/email は `meta.narrative` を消費 |
| Orchestrator | `scripts/daily_pipeline.ps1` (idempotent 4 段, 失敗時 ntfy WARN) |
...
### コスト / 期間
- **月額 $0** (Polygon free tier + Vercel Hobby + ntfy.sh + SendGrid free)。追加期間: 0 (本日完了)。
...
3. **per-subscriber routing**: `resolve_publishers()` を subscribers DB 参照に。
   `systems[]` フィルタ (subscriber が購読する system のみ配信) と channel 別 (ntfy / LINE / Email / Discord) を有効化。
...
6. **auto-post fan-out (narrator の横展開)**: `common/publishers/twitter.py` (public) と
   `substack.py` (subscriber) の skeleton を本実装。**同じ `meta.narrative` を 1 LLM call の
   出力から self (自家用 ntfy/email) + subscriber (Substack newsletter) + public (X/Twitter 要約) の
   3 用途へ fan-out** する。narrator を pipeline で 1 回だけ呼び、生成した narrative を各 channel が
   それぞれの形式 (tweet 280 字 / newsletter HTML / push) に render するだけなので追加 LLM コストは 0。
   これにより「解説付きシグナル」を集客 (public) → β購読 (subscriber) の導線に載せる。
...
3. **SLA**: 配信時刻保証 (06:30 JST までに配信) / 稼働率。
   監視: pipeline 失敗時の ntfy WARN + `meta.publish_status` (Phase 1 実装済) を PagerDuty/Opsgenie に昇格。
